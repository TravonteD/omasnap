...
|---|---|---|
| `select` | `V` | Select/move layers |
| `arrow` | `A` | Arrow |
| `line` | `L` | Straight line |
| `freehand` | `F` | Freehand stroke |
| `highlighter` | `H` | Highlighter |
| `marker` | `C`, `M` | Numbered marker |
| `rectangle` / `ellipse` | `R` / `E` | Shapes; press again to toggle fill |
| `spotlight` | `S` | Spotlight/loupe; again cycles shape |
| `redact` | `D` | Redact; again toggles pixelate/solid |
| `cut` | `X` | Cut out a band |
| `text` | `T` | Text; again toggles the pill |
| `eyedropper` | `I` | Sample a custom color from the image |
| `ocr` | `O` | Copy all text in the image |
| `pin` | `P` | Pin the capture on screen |
| `backdrop` | `B` | Cycle backdrop |
| `duplicate` | `Alt+D` | Duplicate selected layer |
| `undo` / `redo` | `Ctrl+Z` / `Ctrl+Shift+Z`, `Ctrl+Y` | Undo / redo |
| `copy` / `save` | `Ctrl+C` / `Ctrl+S` | Finish with that output only |
| `select-all` | `Ctrl+A` | Select everything (or the full monitor) |
| `zoom-in` / `zoom-out` / `zoom-fit` | `+`, `=`, `Ctrl+=` / `-`, `_`, `Ctrl+-` / `0` | View zoom |
| `color1` … `color8` | `1` … `8` | Palette color slots |
| `restore-region` | `R` | Restore the last region drawn this session (select phase) |
| `scroll-mode` | `S` | Toggle scrolling-region mode (select phase) |
| `cycle-tab` | `Space` | Step through the capture-kind tabs |

Pin-window keys (`Esc`, `Ctrl+C`) and the scrolling-capture panel keys are not
configurable.
